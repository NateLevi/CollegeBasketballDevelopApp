...
export const convertArrayToBasketballStats = (rawData: any[][]): BasketballStats[] => {
...
// Make request to Bart Torvik API and passing in data into the convertArrayToBasketballStats function
export const getBasketballStats = async (): Promise<BasketballStats[]> => {
    const response = await fetch('https://barttorvik.com/getadvstats.php?year=2025&json=1');
...
// Get basketball stats for a specific year
export const getBasketballStatsByYear = async (year: number): Promise<BasketballStats[]> => {
    const response = await fetch(`https://barttorvik.com/getadvstats.php?year=${year}&json=1`);
    const rawData = await response.json() as any[][];
...
// Get player progression across multiple years by player name
export const getPlayerProgression = async (playerName: string, startYear: number = 2022): Promise<BasketballStats[]> => {
    const currentYear = 2025;
    const progression: BasketballStats[] = [];
    
    // Fetch data for each year from startYear to current year
    for (let year = startYear; year <= currentYear; year++) {
        try {
            const yearData = await getBasketballStatsByYear(year);
            const playerData = yearData.find(player => 
                player.player_name.toLowerCase() === playerName.toLowerCase()
            );
            
            if (playerData) {
                progression.push(playerData);
            }
        } catch (error) {
